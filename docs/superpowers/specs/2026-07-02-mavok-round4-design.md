...
- Each `AttackRow` gets a small "⋯" affordance (consistent with the kebab-menu pattern already used in `QuickNotes.tsx`) offering "Editar" (opens `AttackFormModal` pre-filled via `existingAttack`) and "Eliminar" (calls `removeAttack`, with the same undo-toast pattern established in round 3's Task 2/3 — `toast(..., { action: { label: "Deshacer", onClick: () => addAttack(attack) } })`). As with round 3's inventory-delete undo, `addAttack` appends rather than reinserting at the original array position — acceptable, since attacks have no manual reordering UI and display order isn't semantically meaningful today.
  - **Event propagation**: `AttackRow.tsx`'s outer row `<div>` already has `onClick={() => setExpanded(!expanded)}`, and its existing Hit/Dmg buttons each call `e.stopPropagation()` in their own `onClick` specifically to avoid also toggling the row's expand state. The new "⋯" button (placed alongside Hit/Dmg, inside the same `<div className="flex gap-1.5 ml-2">`) must follow the identical `e.stopPropagation()` pattern, or tapping it will also collapse/expand the row underneath.
...
- `LevelUpFlow.tsx`'s `applyAll()` appends one `LevelUpHistoryEntry` per completed level-up (`date: new Date().toISOString()`). The existing `PendingChanges` state (`LevelUpFlow.tsx:22-33`) already captures everything needed: `changes.abilityIncreases: Partial<Record<AbilityScore, number>>` becomes `asiChoice` as a formatted summary string when non-empty — e.g. `"FUE +2"` or `"FUE +1, CON +1"`, built from `Object.entries(changes.abilityIncreases).map(([ab, inc]) => \`${abilityLabelShort(ab)} +${inc}\`).join(", ")`. Note this uses `abilityLabelShort` (short codes like "FUE"/"CON"), not the `abilityLabel` (full names like "Fuerza") the existing Summary-step list at `LevelUpFlow.tsx:419-424` uses per-line — the history log is a single condensed line per level, not a restatement of the Summary step's own per-ability list, so the shorter label reads better here. `changes.feat?.name` becomes `featChosen` when a feat was chosen instead. Both are `undefined` on levels with no ASI step (`ASI_LEVELS = [4, 8, 12, 16, 19]`, `LevelUpFlow.tsx:19`).
