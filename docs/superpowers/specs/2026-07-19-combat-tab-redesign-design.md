...
**The token *values* do change, though** — this needs to be explicit, since an earlier draft of this section implied otherwise while the Context table above it already said "deeper texture, better contrast" for direction A. Those aren't compatible unless the underlying hex values move.

The approved mockups are the source of truth for the new values, preserved alongside this spec at `docs/superpowers/specs/2026-07-19-combat-tab-redesign-mockups/` (**not** `.superpowers/brainstorm/...` — that path is gitignored session/scratch state and isn't guaranteed to exist by the time this gets implemented; an earlier draft of this doc pointed there, which was itself a bug this self-review caught):

- `piedra-viva` ← `combat-fixes.html`, `.prev-a`/`.fix-box` rules (background `#14100c`→`#241d16`, card gradient `#1a140e`→`#2a2016`, border `#4a3520`, accent-as-value-text `#c98a4b`, cord unchanged at `#a63d2f`).
...
These were visual sketches approved for *direction*, not pixel-measured production values — implementation should extract concrete numbers from those files' `<style>` blocks (same mechanical step `2026-07-04-new-themes-design.md` did with its inline palette table, just sourced from preserved HTML instead of restated in Markdown here, to avoid a second transcription that could drift from what was actually approved) and use judgment on the exact final values within that direction. **Verify text contrast** (`--fg`/`--muted` against `--bg`/`--card`) still holds at roughly WCAG AA (4.5:1 for body text) once real values land, especially for `--muted` text at the 11px floor below — mockups were checked visually, not measured, and darker/richer tones can quietly fail contrast where the current flatter tones didn't.
...
- **The existing `density: "compact" | "spacious"` setting (`useThemeContext().density`) applies here too.** `AttackRow.tsx` already branches its padding on this (`density === "compact" ? "p-2" : "p-3"`), as do the Inventory/Notes list rows — new compact-row components must follow the same pattern rather than hardcoding one density and silently overriding a preference that already exists. The 40px floor above is the `compact` value specifically; `spacious` gets more generous padding as it already does elsewhere.
...
- **⚔ Atacar** scrolls to and expands `attacks[0]`'s row (the first attack in display order — the same ordering `moveAttack` already controls), landing the user directly on its Hit/Dmg buttons — the same "bring the section to the user" mechanism as "Roll rápido" below, not a direct roll trigger. (An earlier draft of this spec said it "rolls" the primary attack directly; that's not achievable without lifting `AttackRow`'s roll state — which is local to that component today — out into `CombatTab`, which is more invasive plumbing than one shortcut justifies. Scroll-and-expand achieves the same goal — skip the scroll past conditions/exhaustion — without it.) **When `attacks.length === 0`, this quick action is hidden**, not shown disabled — a character with no attacks defined yet is already served by the "+ Agregar ataque" row in Acciones, and a dead/disabled shortcut adds visual noise without a path forward.
...
- Quick-actions row scrolls to and expands `attacks[0]` / the Dados section correctly; "Atacar" is absent (not disabled) on a character with zero attacks defined.
