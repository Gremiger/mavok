...
- **The browsable list excludes two categories of feat entirely**, both for the same reason: `LevelUpFlow.tsx`'s existing `getEligibleFeats()` (`LevelUpFlow.tsx:280-295`) already treats `category === "Fighting Style"` and `requiresSpellcasting` identically — both are hard `return false` exclusions, not soft prereq checks like `abilityPrereqs`. Neither is something Mavok can ever take: Fighting Style feats aren't choosable via the general ASI/feat slot for a Barbarian at all, and Mavok has no path to spellcasting anywhere in this app (no multiclass support, and none of the 4 XPHB Barbarian subclasses grant spellcasting). Showing either as "locked"/"unavailable" would be clutter, not useful planning information — the browser excludes both from the list entirely, consistent with how the existing code already treats them.
- For the remaining feats, the browser computes a **per-feat status label**, not a binary include/exclude filter (unlike `getEligibleFeats`, which excludes ineligible feats entirely) — since the whole point is showing what's still locked:
...
- Filter chips by the remaining categories (General / Origin / Epic Boon), matching the filter-chip pattern already used in `QuestList.tsx` (status filter) and `InventoryTab.tsx` (category filter).
